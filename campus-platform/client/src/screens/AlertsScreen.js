      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <View style={S.alertTopRow}>
            <Text style={[S.alertType, { color: item.color }]}>
              {item.type}
            </Text>
...
export default function AlertsScreen({ navigation }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      // Append the static seed at the end so the screen never feels empty if backend is offline.
      ...
      setAlerts(ALERTS); // graceful fallback
    } finally {
      setLoading(false);
    }
  };
