...
const Orbs = () => (
  <>
    <View style={[s.orb, { backgroundColor: C.primary, top: -140, right: -130, width: 340, height: 340, borderRadius: 170 }]} />
    <View style={[s.orb, { backgroundColor: C.red,     bottom: -80, left: -90,  width: 260, height: 260, borderRadius: 130, opacity: 0.04 }]} />
  </>
);
...
const InputBox = ({
  ...
  secure, onToggle, showToggle,
  keyboardType, matchState, editable = true,
}) => {
  ...
  const borderColor = matchState === 'match'
    ? C.white
    : matchState === 'mismatch'
    ? C.red
    : anim.interpolate({ inputRange: [0, 1], outputRange: [C.inputBorder, C.primary] });
...
const AlertBox = ({ type, msg }) => {
  ...
  return (
    <View style={[s.alertBox, { backgroundColor: bg, borderColor: text + '44' }]}>
...
function ForgotPasswordScreen({ navigate }) {
  ...
  const handleOtpChange = (val, idx) => {
    ...
    if (digit && idx < OTP_LENGTH - 1) otpRefs.current[idx + 1]?.current?.focus();
  ...
  const verifyOtp = async () => {
    ...
    if (entered.length < OTP_LENGTH) return setErr('⚠️  Enter the 6-digit OTP.');
            ...
            <View style={s.resendRow}>
              {timer > 0 ? (
                <Text style={s.timerTxt}>
                  Resend OTP in{' '}
                  <Text style={{ color: C.white }}>0:{timer.toString().padStart(2, '0')}</Text>
                </Text>
              ) : (
...
function ForceResetScreen({ navigate, params }) {
  ...
  const strengthColor = strength <= 1 ? C.red : strength <= 2 ? C.amber : C.white;
  const strengthLabel = strength <= 1 ? '⚠️  Weak' : strength <= 2 ? '⚡  Medium' : '✅  Strong';
  const segmentColors = [0, 1, 2, 3].map((i) => (i < strength ? strengthColor : C.grey3));
  ...
  const handleReset = async () => {
    setErr('');
<<<<<<< HEAD:campus-platform/client/src/screens/ResetPassword.jsx
    ...
    try {
      ...
      Alert.alert(
        ...
        [{ text: 'Go to Login', onPress: async () => { await apiLogout(); navigate('Login'); } }]
      );
    } catch (err) {
      setLoading(false);
...
=======
    ...
    try {
      const res  = await fetch(`${API_BASE}/reset-password`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ email: params?.email, phone: params?.phone, newPassword: newPwd }),
      });
      const data = await res.json();
      ...
      setLoading(false);
>>>>>>> 42497444c3dfa972ccb0e3bbcafe0428cec6335a:client/src/screens/ResetPassword.jsx
    }
  };
      ...
      <Animated.View style={[s.warningBanner, { opacity: fade }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.warningTxt}>
            You're using default credentials. Please set a new secure password to continue.
          </Text>
        </View>
      </Animated.View>
      ...
      <Animated.View style={[s.card, { opacity: fade, transform: [{ translateY: slide }] }]}>
        ...
        {newPwd.length > 0 && (
          <View style={s.strBlock}>
            ...
            <Text style={[s.strLabel, { color: strengthColor }]}>{strengthLabel} password</Text>
          </View>
        )}
            ...
            {PASSWORD_REQUIREMENTS.map((req, i) => {
              ...
              return (
                <View key={i} style={s.reqRow}>
                  <View style={[s.reqDot, met && { backgroundColor: C.white }]} />
                  <Text style={[s.reqTxt, met && { color: C.white }]}>{req.label}</Text>
                </View>
              );
            })}
          </View>
        ...
        />
        {isMatch    && <Text style={[s.matchTxt, { color: C.white }]}>✅  Passwords match</Text>}
...
>>>>>>> 42497444c3dfa972ccb0e3bbcafe0428cec6335a:client/src/screens/ResetPassword.jsx
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setParams(prev => ({ ...(prev || {}), email: stored.email, ...(route?.params || {}) }));
    })();
    return () => { cancelled = true; };
  }, []);

  // Bridge the file's internal `navigate(to)` callback API onto react-navigation.
  const navigate = useCallback((to, _p = null) => {
...
const s = StyleSheet.create({
  ...
  logoRing:   { width: 90, height: 90, borderRadius: 45, borderWidth: 2, borderColor: C.primary, alignItems: 'center', justifyContent: 'center', marginBottom: 12, shadowColor: C.primary, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.6, shadowRadius: 16, elevation: 10 },
  logoCircle: { width: 70, height: 70, borderRadius: 35, backgroundColor: C.card, alignItems: 'center', justifyContent: 'center' },
  logoImage:  { width: 50, height: 50 },

  // Title block
  titleBlock: { alignItems: 'center', marginBottom: 20, width: '100%' },
  pageTitle:  { fontSize: 28, fontWeight: '800', color: C.white, letterSpacing: 0.3, marginBottom: 10, textAlign: 'center' },
  accent:     { width: 44, height: 3, borderRadius: 2, backgroundColor: C.primary, marginBottom: 10 },
  pageSub:    { fontSize: 13.5, color: C.grey1, textAlign: 'center', lineHeight: 21 },

  // Card
  card: { width: '100%', backgroundColor: C.card, borderRadius: 24, padding: 24, marginBottom: 20, borderWidth: 1, borderColor: C.cardBorder, shadowColor: C.primary, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.25, shadowRadius: 20, elevation: 18 },
  ...
  inputBoxFocused: { shadowColor: C.primary, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
  ...
  // Primary button
  primaryBtn:     { borderRadius: 14, paddingVertical: 16, alignItems: 'center', backgroundColor: C.primary, shadowColor: C.primary, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.55, shadowRadius: 14, elevation: 10, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.15)' },
  btnDisabled:    { opacity: 0.6 },
  primaryBtnText: { color: C.white, fontSize: 15.5, fontWeight: '700', letterSpacing: 1.4, textTransform: 'uppercase' },
  ...
  otpBox:         { width: (width - 44 - 48 - 50) / 6, height: 54, borderRadius: 12, borderWidth: 2, borderColor: C.grey3, backgroundColor: C.input, color: C.white, fontSize: 22, fontWeight: '700', textAlign: 'center' },
  ...
  // Logout
  logoutTxt: { color: C.red, textAlign: 'center', marginTop: 10, fontSize: 14, fontWeight: '600' },
  ...
  warningTitle:  { color: C.primaryLight, fontSize: 13, fontWeight: '700', marginBottom: 3 },
