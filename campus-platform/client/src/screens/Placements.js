    ...
    <Animated.View style={[anim, EC.card]}>
      <View style={EC.header}>
        ...
        <View style={{ alignItems: 'flex-end', gap: 5 }}>
          <View style={EC.datePill}>
            <Text style={EC.dateTxt}>{exp.date}</Text>
          </View>
      ...
      <Text style={EC.preview} numberOfLines={expanded ? undefined : 3}>
        {exp.experience}
      </Text>
...
// ─── SubmissionCard (live backend submissions) ────────────────────────────────

const SubmissionCard = ({ sub }) => (
  <View style={SC.card}>
    <View style={SC.topRow}>
      <Text style={SC.username} numberOfLines={1}>{sub.Username || 'Anonymous'}</Text>
      <Text style={SC.date}>
        {sub.createdAt ? new Date(sub.createdAt).toLocaleDateString() : ''}
      </Text>
    </View>
    <Text style={SC.meta}>{sub.Position || 'Role'} · {sub.Company || 'Company'}</Text>
    <Text style={SC.description} numberOfLines={4}>{sub.description || ''}</Text>
  </View>
);

const SC = StyleSheet.create({
  card:        { backgroundColor: C.surface, borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: C.border, shadowColor: '#00000015', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 1, shadowRadius: 6, elevation: 2 },
  topRow:      { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  username:    { fontWeight: '800', color: C.textDark, fontSize: 14 },
  date:        { color: C.textLight, fontSize: 11 },
  meta:        { color: C.textMid, fontSize: 12, marginBottom: 6 },
  description: { color: C.textMid, fontSize: 12, lineHeight: 18 },
});

// ─── UploadModal ──────────────────────────────────────────────────────────────

const UploadModal = ({ onClose, onSubmitted }) => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await placementsApi.create({
        Username:    name.trim(),
        Company:     company,
        Position:    role.trim(),
      ...
      if (typeof onSubmitted === 'function') onSubmitted(result?.data);

      Alert.alert(
        'Submitted',
        'Your experience was uploaded. Thank you for helping juniors!',
        [{ text: 'OK', onPress: close }],
      );
    } catch (err) {
      Alert.alert('Submission Failed', err?.message || 'Could not reach the server.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        >
              ...
              <ScrollView style={{ maxHeight: 180 }} nestedScrollEnabled>
                {COMPANY_NAMES.map((cn) => (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.75}
                  >
                    <Text style={[UM.dropItemTxt, company === cn && { color: C.primary, fontWeight: '700' }]}>
                      {cn}
                    </Text>
          ...
          <View style={UM.diffRow}>
            {DIFFICULTY_OPTIONS.map((d) => {
              const { bg, fg } = DIFFICULTY_COLORS[d];
          ...
          {/* Tips */}
          <Text style={UM.label}>
            Preparation Tips <Text style={UM.optional}>(optional)</Text>
          </Text>
          <TextInput
            style={[UM.input, UM.textarea]}
          ...
          {/* Submit */}
          <TouchableOpacity onPress={handleSubmit} disabled={submitting} activeOpacity={0.88} style={{ marginTop: 8 }}>
            <LinearGradient colors={['#1E3A8A', '#2563EB']} style={UM.submitBtn}>
              <Ionicons name="cloud-upload-outline" size={18} color="#fff" />
              <Text style={UM.submitTxt}>{submitting ? 'Submitting...' : 'Submit Experience'}</Text>
            </LinearGradient>
          </TouchableOpacity>
      ...
      <Animated.View style={[BS.sheet, { transform: [{ translateY: slideY }] }]}>
        ...
        <View style={BS.header}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, flex: 1 }}>
            ...
            <View>
              ...
              <Text style={BS.expCount}>
                {exps.length} student experience{exps.length !== 1 ? 's' : ''}
              </Text>
            </View>
          </View>
        ...
        >
          {exps.length === 0 ? (
            ...
            exps.map((exp, i) => <ExperienceCard key={exp.id} exp={exp} index={i} />)
          )}
        </ScrollView>
      </Animated.View>
    </Modal>
...
const CompanyCard = ({ company, onPress, index }) => {
  const anim  = useEntrance(index * 60, 18);
  const press = useRef(new Animated.Value(1)).current;

  const onPressIn  = () => Animated.spring(press, { toValue: 0.97, speed: 22, useNativeDriver: true }).start();
  const onPressOut = () => Animated.spring(press, { toValue: 1,    speed: 16, useNativeDriver: true }).start();
...
export default function PlacementsScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await placementsApi.list();
        if (!cancelled) setSubmissions(res?.data || []);
      } catch (err) {
        if (!cancelled) setSubmissionsErr(err?.message || 'Failed to load submissions');
      }
    })();
  ...
  return (
    <View style={S.root}>
      ...
      >
        ...
        {/* ── Recent student submissions (live) ── */}
        {submissions.length > 0 && (
          <Animated.View style={[listAnim, { paddingHorizontal: 18, marginBottom: 6 }]}>
            <Text style={[S.sectionTitle, { marginBottom: 8 }]}>Recent Student Submissions</Text>
            {submissions.slice(0, 5).map((sub) => (
              <SubmissionCard key={String(sub._id || sub.Username + sub.createdAt)} sub={sub} />
            ))}
          </Animated.View>
        )}

        {!!submissionsErr && submissions.length === 0 && (
          <Animated.View style={[listAnim, { paddingHorizontal: 18, marginBottom: 6 }]}>
            <Text style={S.submissionsErr}>
              Could not load student submissions: {submissionsErr}
            </Text>
          </Animated.View>
        )}
...
const S = StyleSheet.create({
  ...
  // Submissions error
  submissionsErr: { color: C.textLight, fontSize: 11, fontStyle: 'italic' },
