...
const UploadModal = ({ onClose, onSubmitted }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Submitted', 'Your experience was uploaded. Thank you for helping juniors!', [
      ...
      <Animated.View style={[UM.sheet, { transform:[{ translateY:slideY }] }]}>
        ...
        >
          ...
          <TextInput
            ...
            placeholderTextColor={C.textLight}
            value={experience}
            onChangeText={(text) => { setExperience(text); setSummarized(false); }ulty <Text style={UM.required}>*</Text></Text>
  ...
  return (
    <View style={S.root}>
      ...
      >
        ...
        {/* ── RECENT STUDENT SUBMISSIONS (live from backend) ── */}
        {submissions.length > 0 && (
          <Animated.View style={[listAnim, { paddingHorizontal:18, marginBottom:6 }]}>
            ...
            {submissions.slice(0, 5).map((sub) => (
              <View
                key={String(sub._id || sub.Username + sub.createdAt)}
                style={{
                  backgroundColor:C.surface, borderRadius:14, padding:14, marginBottom:10,
                  borderWidth:1, borderColor:C.border,
                  shadowColor:'#00000015', shadowOffset:{ width:0, height:2 },
                  shadowOpacity:1, shadowRadius:6, elevation:2,
                }}
              >
                <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                  <Text style={{ fontWeight:'800', color:C.textDark, fontSize:14 }} numberOfLines={1}>
                ...
                <Text style={{ color:C.textMid, fontSize:12, marginBottom:6 }}>
                  {sub.Position || 'Role'} · {sub.Company || 'Company'}
                </Text>
                <Text style={{ color:C.textMid, fontSize:12, lineHeight:18 }} numberOfLines={4}>
                  {sub.description || ''}
                </Text>
              </View>
            ))}
        ...
        {!!submissionsErr && submissions.length === 0 && (
          <Animated.View style={[listAnim, { paddingHorizontal:18, marginBottom:6 }]}>
            <Text style={{ color:C.textLight, fontSize:11, fontStyle:'italic' }}>
      ...
      {showUpload && (
        <UploadModal
          ...
          onSubmitted={(created) => created && setSubmissions((prev) => [created, ...prev])}
        />
      )}
    </View>
  );
