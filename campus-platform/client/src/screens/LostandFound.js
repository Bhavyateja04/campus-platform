...
function backendItemToUi(it) {
  ...
  return {
    ...
    personName: it.postedByName || 'Anonymous',
    rollNo: it.postedByRoll || '',
...
export default function LostAndFoundScreen({ navigation }) {
  ...
  const addItem = useCallback(async (item) => {
    ...
    try {
      ...
      const created = backendItemToUi(result?.data);
      if (created) {
        const setter = created.type === 'lost' ? setLostItems : setFoundItems;
        setter(prev => {
          const replaced = prev.map(p => (p.id === item.id ? created : p));
          if (!replaced.some(p => p.id === created.id)) return [created, ...replaced];
          return replaced;
        });
      }
    } catch (err) {
      console.warn('[LostAndFound] backend create failed:', err?.message || err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (lost.length)  setLostItems(prev => [...lost, ...prev.filter(p => !p._backend)]);
        if (found.length) setFoundItems(prev => [...found, ...prev.filter(p => !p._backend)]);
      } catch (err) {
        console.warn('[LostAndFound] backend fetch failed (using seed data):', err?.message || err);
      }
    })();
