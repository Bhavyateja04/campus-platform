  ...
  const loadAdminData = async () => {
    try {
      const [
        ...
        apiRequest("/api/admin/viewusers").catch(() => null),
        apiRequest("/api/lostitems").catch(() => null),
        apiRequest("/api/goods").catch(() => null),
        apiRequest("/api/canteens").catch(() => null),
        apiRequest("/api/notifications").catch(() => null),
        apiRequest("/api/placements").catch(() => null),
        apiRequest("/api/college-memories/all-memories").catch(() => null),
        apiRequest("/api/clubs").catch(() => null),
        apiRequest("/api/exam-search").catch(() => null),
      ]);
      ...
      setNotifications(normalizeNotifications(normalizeList(notificationsRes)));
...
function UsersPage() {
  ...
  const deleteUser = async (user) => {
    try {
      await apiRequest(`/api/admin/deleteuser/${user.id}`, {
        method: "DELETE",
      });
      ...
      console.warn(
        "User delete fell back to local state:",
        error.message || error,
      );
    }
    setSelected(null);
...
function ProfileDetails({ user, onDelete }) {
  return (
...
function LostFoundPage() {
  ...
  const deletePost = async (item) => {
    try {
      await apiRequest(`/api/lostitems/${item.id}`, { method: "DELETE" });
      await reloadAdminData();
      showToast("Lost & Found post deleted");
    } catch (error) {
      setPosts((current) => current.filter((post) => post.id !== item.id));
      showToast("Lost & Found post deleted locally");
      console.warn(
        "Lost item delete fell back to local state:",
        error.message || error,
      );
    }
  };
  const markResolved = async (item) => {
    try {
      await apiRequest(`/api/lostitems/${item.id}/resolved`, {
        method: "PATCH",
      });
      await reloadAdminData();
      ...
      setPosts((current) =>
        current.map((post) =>
          post.id === item.id ? { ...post, status: "resolved" } : post,
        ),
      );
      showToast("Marked as resolved locally");
      console.warn(
        "Lost item resolve fell back to local state:",
        error.message || error,
      );
    }
  };
...
function MarketplacePage() {
  ...
  const markSold = async (product) => {
    try {
      await apiRequest(`/api/goods/${product.id}/sold`, { method: "PATCH" });
      await reloadAdminData();
    } catch (error) {
      setItems((current) =>
        current.map((item) =>
          item.id === product.id ? { ...item, status: "sold" } : item,
        ),
      );
      console.warn(
        "Marketplace sold update fell back to local state:",
        error.message || error,
      );
    }
  };
  const deleteProduct = async (product) => {
    try {
      await apiRequest(`/api/goods/${product.id}`, { method: "DELETE" });
      await reloadAdminData();
    } catch (error) {
      setItems((current) => current.filter((item) => item.id !== product.id));
      console.warn(
        "Marketplace delete fell back to local state:",
        error.message || error,
      );
    }
  };
...
function PlacementsPage() {
  ...
  const deletePlacement = async (job) => {
    try {
      await apiRequest(`/api/placements/${job.id}`, { method: "DELETE" });
      await reloadAdminData();
    } catch (error) {
      setPosts((current) => current.filter((post) => post.id !== job.id));
      console.warn(
        "Placement delete fell back to local state:",
        error.message || error,
      );
    }
    setPendingDelete(null);
...
function MemoriesPage() {
  ...
  const pushMemoryUpdate = async (memory, patch) => {
    ...
    try {
      await apiRequest(`/api/college-memories/edit-memory/${memory.id}`, {
        method: "PUT",
        body: nextPatch,
      });
      await reloadAdminData();
    } catch (error) {
      updateMemory(memory.caption, patch);
      console.warn(
        "Memory update fell back to local state:",
        error.message || error,
      );
    }
  };
  const removeMemory = async (memory) => {
    try {
      await apiRequest(`/api/college-memories/delete-memory/${memory.id}`, {
        method: "DELETE",
      });
      await reloadAdminData();
    } catch (error) {
      deleteMemory(memory.caption);
      console.warn(
        "Memory delete fell back to local state:",
        error.message || error,
      );
    }
  };
