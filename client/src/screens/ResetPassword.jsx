...
// ─── DEFAULT CREDENTIALS ─────────────────────────────────────────────────────
const DEFAULT_EMAIL    = 'student@campus.edu';
const DEFAULT_PASSWORD = 'Campus@123';
const DEMO_OTP         = '847291';

// ─── THEME — #D00000 Red + White ─────────────────────────────────────────────
const C = {
  bg:          '#1A0000',
  card:        '#2A0000',
  cardBorder:  '#4A0000',
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      if (email.trim().toLowerCase() === DEFAULT_EMAIL && password === DEFAULT_PASSWORD) {
        ...
        Alert.alert('✅ Login Successful', 'Welcome to Campus System!');
      } else {
        setErr('⚠️  Invalid credentials. Please try again.');
      }
    }, 1500);
  };
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={s.container}>
          ...
          <Animated.View style={[s.card, { opacity: fade, transform: [{ translateY: slide }] }]}>
            ...
            <InputBox icon="✉️" placeholder="yourname@university.edu" value={email} onChangeText={setEmail} keyboardType="email-address" />

            <View style={{ height: 14 }} />
            <FieldLabel>Password</FieldLabel>
            <InputBox icon="🔒" placeholder="Enter your password" value={password} onChangeText={setPassword} secure={!showPwd} showToggle onToggle={() => setShowPwd(!showPwd)} />
          ...
          <Animated.View style={[s.hintBox, { opacity: fade }]}>
            <Text style={s.hintTitle}>🧪 Demo Credentials</Text>
            <Text style={s.hintTxt}>Default login: <Text style={{ color: C.primaryLight }}>{DEFAULT_EMAIL}</Text></Text>
            <Text style={s.hintTxt}>Password: <Text style={{ color: C.primaryLight }}>{DEFAULT_PASSWORD}</Text></Text>
            <Text style={[s.hintTxt, { marginTop: 4, color: C.amber }]}>↑ Forces password reset flow</Text>
          </Animated.View>
        </View>
      </ScrollView>
...
function ForgotPasswordScreen({ navigate }) {
  ...
  const sendOtp = () => {
    setErr('');
    if (!email.trim())        return setErr('⚠️  Please enter your email address.');
    if (!email.includes('@')) return setErr('⚠️  Enter a valid email address.');
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      setStep('otp');
      setTimer(60);
      setInfo(`OTP sent to ${email}. (Demo OTP: ${DEMO_OTP})`);
    }, 1500);
  };
  ...
  const verifyOtp = () => {
    ...
    if (entered.length < 6)   return setErr('⚠️  Please enter the complete 6-digit OTP.');
    if (entered !== DEMO_OTP) return setErr('⚠️  Invalid OTP. Please try again.');
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate('ForceReset', { email }); }, 1200);
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={s.container}>
          ...
          <Animated.View style={[s.titleBlock, { opacity: fade, transform: [{ translateY: slide }] }]}>
            <Text style={s.pageTitle}>{step === 'email' ? 'Forgot Password' : 'Enter OTP'}</Text>
            <View style={[s.accent, { backgroundColor: '#FFFFFF' }]} />
            <Text style={s.pageSub}>
              {step === 'email' ? 'Enter your registered email to receive a verification code' : `We sent a 6-digit code to\n${email}`}
            </Text>
          </Animated.View>
          ...
          <Animated.View style={[s.card, { opacity: fade, transform: [{ translateY: slide }] }]}>
            ...
            {step === 'email' ? (
              ...
              <>
                ...
                <TouchableOpacity style={s.changeEmailRow} onPress={() => { setStep('email'); setErr(''); setInfo(''); setOtp(['','','','','','']); }}>
                  <Text style={s.changeEmailTxt}>← Change Email</Text>
                </TouchableOpacity>
              </>
            )}
          </Animated.View>
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={s.container}>
          ...
          <Animated.View style={[s.card, { opacity: fade, transform: [{ translateY: slide }] }]}>
            ...
            <InputBox icon="✉️" placeholder="" value={params?.email || DEFAULT_EMAIL} editable={false} />
            ...
            <View style={{ height: 24 }} />
            <PrimaryBtn label="🔐  Confirm & Update Password" onPress={handleReset} loading={loading} />
          </Animated.View>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
...
// ─── NAVIGATOR ───────────────────────────────────────────────────────────────
export default function CampusAuthFlow() {
  const [screen, setScreen] = useState('Login');
  const [params, setParams] = useState(null);
  const navigate = useCallback((to, p = null) => { setParams(p); setScreen(to); }, []);
  return (
